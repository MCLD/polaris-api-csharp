...
namespace Clc.Polaris.Api
{
	public partial class PolarisApiClient
	{
		...
		public HoldRequestActivationResult HoldRequestReactivate(string patronBarcode, int requestID, DateTime activationDate, string patronPIN)
		{
			...
			var request =
				new RestRequest(string.Format("public/v1/1033/100/1/patron/{0}/holdrequests/{1}/active", patronBarcode, requestID),
				                Method.PUT);
		...
		/// <returns>An error code and message from the API, if any.</returns>
		/// <seealso cref="HoldRequestCancelResult"/>
		public HoldRequestActivationResult staff_HoldRequestReactivate(string patronBarcode, int requestID, DateTime activationDate)
		{
			var doc = new XDocument(
				new XElement("HoldRequestActivationData",
				             new XElement("UserID", 1),
